...
function replaceText(word, replacement, text) {
  ...
  const changedText = text.replace(regex, replacement);
...
function checkWord(word, text) {
  ...
  const WordPresent = regex.test(text);
...
function countEmails(str) {
  ...
  const numberOfEmails = matches ? matches.length : 0;
...
function checkRegexFlags(regex) {
  ...
  const getFlags = regex.flags;

  const hasFirstFlag = getFlags.includes("g");
  const hasSecondFlag = getFlags.includes("m");

  return hasFirstFlag && hasSecondFlag;
}
...
function findWord(str, word) {
  ...
  const firstAppearance = str.search(regex);
